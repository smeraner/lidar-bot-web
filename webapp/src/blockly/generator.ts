...
export function defineGenerators() {
    ...
    javascriptGenerator.forBlock['lidarbot_move'] = function (block: any) {
        ...
        let moveCode = '';
        if (direction === 'FORWARD') moveCode = `await serialBridge.sendCommand(0, ${sCode}, 0, ${duration});`;
        if (direction === 'BACKWARD') moveCode = `await serialBridge.sendCommand(0, -(${sCode}), 0, ${duration});`;
        if (direction === 'LEFT') moveCode = `await serialBridge.sendCommand(-(${sCode}), 0, 0, ${duration});`;
        if (direction === 'RIGHT') moveCode = `await serialBridge.sendCommand(${sCode}, 0, 0, ${duration});`;
